...
const useAuth = () => {
  ...
  const signUp = React.useCallback((email: string, name: string, password: string, repeatPassword: string, avatar: File | undefined) => {
    dispatch(signUpAsyncAction({email: email, name: name, password: password, repeatPassword: repeatPassword, avatar: avatar}));
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
